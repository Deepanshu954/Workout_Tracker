...
export const WeightChart = ({ entries }: WeightChartProps) => {
      ...
      <Card className="shadow-card">
        ...
        <CardContent>
          <div className="flex items-center justify-center h-64 text-muted-foreground">
            No weight entries yet. Add your first entry to see progress!
          </div>
        </CardContent>
      </Card>
    );
  ...
  const getTrendIcon = () => {
    if (weightChange > 0) return <TrendingUp className="h-4 w-4 text-warning" />;
    if (weightChange < 0) return <TrendingDown className="h-4 w-4 text-success" />;
